...
def evaluate_gates(
    *,
    ...
    for gate_name, metric_name in MIN_GATES.items():
        checks.append(
            _check_min(gate_name, eval_report.get(metric_name, 0), gates[gate_name])
        )

    for gate_name, metric_name in MAX_GATES.items():
        checks.append(
            _check_max(gate_name, eval_report.get(metric_name, 0), gates[gate_name])
        )
...
def _check_min(gate_name: str, actual: Any, threshold: Any) -> dict[str, Any]:
    return {
        "gate": gate_name,
        "actual": actual,
        "operator": ">=",
